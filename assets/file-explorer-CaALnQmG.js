import{c as p}from"./window-manager-Cm3UCJuh.js";import{f as u}from"./data-CMMTyLTn.js";function y(o){o.style.cssText='padding:0;font-family:"JetBrains Mono",monospace;font-size:13px;display:flex;flex-direction:column;';let r="~";function t(i){r=i;const s=c(i);if(!s||s.type!=="dir")return;const f=i.split("/").map((e,n,d)=>`<span class="breadcrumb" data-path="${d.slice(0,n+1).join("/")}" style="cursor:pointer;color:#3b82f6;">${e}</span>`).join(' <span style="color:#555;">/</span> ');o.innerHTML=`
        ...
        `,o.querySelectorAll(".file-item").forEach(e=>{e.addEventListener("dblclick",()=>{const n=e.dataset.name;if(e.dataset.type==="dir")t(r+"/"+n);else if(n==="resume.pdf")window.open("/resume.pdf","_blank");else{const a=c(r+"/"+n);a&&a.content&&p({id:"file-"+n,title:n,icon:"📝",width:500,height:350,content:`<pre style="white-space:pre-wrap;color:#aaa;font-family:'JetBrains Mono',monospace;font-size:13px;">${a.content}</pre>`})}}),e.addEventListener("mouseenter",()=>e.style.background="rgba(255,255,255,0.04)"),e.addEventListener("mouseleave",()=>e.style.background="")});const l=o.querySelector(".nav-back");l&&l.addEventListener("click",()=>{const e=r.split("/");e.length>1&&(e.pop(),t(e.join("/")))}),o.querySelectorAll(".breadcrumb").forEach(e=>{e.addEventListener("click",()=>t(e.dataset.path))})}t("~")}function v(){p({id:"projects",title:"File Explorer — ~/projects",icon:"📁",width:600,height:420,content:o=>y(o)})}function c(o){const r=o.replace(/^~\/?/,"").replace(/\/+$/,"");let t=u["~"];if(!r)return t;const i=r.split("/");for(const s of i){if(!t||t.type!=="dir"||!t.children[s])return null;t=t.children[s]}return t}export{v as open,y as renderContent};
