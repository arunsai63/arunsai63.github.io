    ...
    content: (el) => {
      el.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px;">
          ${wallpapers.map((wp, i) => `
            <div class="wp-option" data-idx="${i}" style="cursor:pointer;border-radius:8px;overflow:hidden;border:2px solid #262626;transition:border-color 0.2s;">
              <div style="height:60px;background:${wp.bg};background-image:${wp.gradient};"></div>
              <div style="padding:6px;text-align:center;font-size:11px;color:#888;background:#111;">${wp.name}</div>
            </div>
          `).join('')}
        </div>
        ...
        <div style="background:#111;padding:12px;border-radius:8px;font-family:'JetBrains Mono',monospace;font-size:11px;color:#666;line-height:1.8;">
