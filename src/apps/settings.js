...
import { getYOE, profile } from '../shared/data.js'
...
const wallpapers = [
  ...
  { name: 'Void', bg: '#000', gradient: 'none' },
]
...
export function renderContent(el) {
  ...
  function render() {
    el.innerHTML = `
      <div class="stg">
        <div class="stg-sidebar">
          ...
          ${renderSidebarItem('general', '⚙️', 'General')}
          ${renderSidebarItem('display', '🎨', 'Display')}
          ${renderSidebarItem('about', 'ℹ️', 'About')}
          ${renderSidebarItem('switch', '🔄', 'Switch OS')}
        </div>
    ...
    // Sidebar nav
    el.querySelectorAll('.stg-nav-item').forEach(item => {
    ...
    // Wallpaper clicks
    el.querySelectorAll('.stg-wp-card').forEach(card => {
      card.addEventListener('click', () => {
        const wp = wallpapers[parseInt(card.dataset.idx)]
        const desktop = document.querySelector('.desktop')
        if (desktop) {
          desktop.style.background = wp.bg
          desktop.style.backgroundImage = wp.gradient
        }
        el.querySelectorAll('.stg-wp-card').forEach(c => c.classList.remove('stg-wp-active'))
        card.classList.add('stg-wp-active')
      })
    })

    // OS switch cards
    el.querySelectorAll('.stg-os-card[data-os]').forEach(card => {
      card.addEventListener('click', () => {
        import('../shared/os-detect.js').then(({ switchToOS }) => {
          switchToOS(card.dataset.os)
        })
      })
    })
  }

  function renderSidebarItem(section, icon, label) {
    return `<div class="stg-nav-item ${activeSection === section ? 'stg-nav-active' : ''}" data-section="${section}">
      <span class="stg-nav-icon">${icon}</span><span>${label}</span>
    </div>`
  }
  ...
  function renderGeneral() {
    return `
      ...
      <div class="stg-group">
        ...
        <div class="stg-row"><span class="stg-row-label">Version</span><span class="stg-row-value">ArunOS v${getYOE()}</span></div>
    ...
    return `
      ...
      <div class="stg-group">
        ...
        <div class="stg-wp-grid">
          ${wallpapers.map((wp, i) => `
            <div class="stg-wp-card" data-idx="${i}">
              <div class="stg-wp-preview" style="background:${wp.bg};background-image:${wp.gradient};"></div>
              <div class="stg-wp-name">${wp.name}</div>
            </div>
          `).join('')}
        </div>
      </div>`
  ...
  function renderAbout() {
    return `
      ...
      <div class="stg-info-block">
        ...
        <div><span class="stg-info-label">Uptime:</span> ${getYOE()} years (and counting)</div>
        ...
        <div><span class="stg-info-label">User:</span> ${profile.name}</div>
      </div>
      <div style="text-align:center;color:#444;font-size:11px;margin-top:16px;">
        Built with mass amounts of caffeine by ${profile.name}.<br>
        No frameworks were harmed in the making of this OS.
      </div>`
  }

  function renderSwitch() {
    const currentOS = document.documentElement.dataset.os || 'macos'
    const osOptions = [
      { id: 'macos', icon: '🖥️', name: 'macOS Desktop', desc: 'Dock, windows, widgets, terminal' },
      { id: 'android', icon: '📱', name: 'Android Phone', desc: 'Material You, notifications, app drawer' },
      { id: 'ipados', icon: '📲', name: 'iPadOS Tablet', desc: 'Stage Manager, Control Center, dock' },
    ]
    return `
      ...
      ${osOptions.map(os => `
        <div class="stg-os-card ${os.id === currentOS ? 'stg-os-card-current' : ''}" ${os.id !== currentOS ? `data-os="${os.id}"` : ''}>
          <span class="stg-os-icon">${os.icon}</span>
          <div>
            <div class="stg-os-name">${os.name}${os.id === currentOS ? ' (current)' : ''}</div>
            <div class="stg-os-desc">${os.desc}</div>
          </div>
        </div>
      `).join('')}`
  }
