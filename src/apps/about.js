    ...
    content: (el) => {
      el.innerHTML = `
        <div style="font-family:'JetBrains Mono',monospace;">
          <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;">
            ...
            <div>
              <h1 style="color:#fff;font-size:22px;margin-bottom:2px;">${profile.name}</h1>
              <p style="color:#10b981;font-size:14px;">${profile.title} @ ${profile.company}</p>
              <p style="color:#666;font-size:12px;">${profile.yoe} years of mass caffeinated development</p>
            </div>
          </div>

          <div style="background:#12122a;padding:14px;border-radius:8px;margin-bottom:16px;font-size:13px;line-height:1.7;color:#aaa;">
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
            ${makeTab('🔥 Hot Takes')}
            ${makeTab('🤔 Unpopular Opinions')}
            ${makeTab('📊 Stats')}
          </div>

          <div id="about-tab-content" style="background:#12122a;padding:14px;border-radius:8px;font-size:13px;line-height:1.7;color:#aaa;">
            ${hotTakes()}
          </div>

          <div style="margin-top:16px;display:flex;gap:8px;flex-wrap:wrap;">
            <a href="${profile.github}" target="_blank" style="padding:6px 12px;background:#1a1a30;border:1px solid #333;border-radius:6px;font-size:12px;color:#aaa;transition:all 0.15s;">🐙 GitHub</a>
            <a href="${profile.linkedin}" target="_blank" style="padding:6px 12px;background:#1a1a30;border:1px solid #333;border-radius:6px;font-size:12px;color:#aaa;transition:all 0.15s;">💼 LinkedIn</a>
            <a href="${profile.blog}" target="_blank" style="padding:6px 12px;background:#1a1a30;border:1px solid #333;border-radius:6px;font-size:12px;color:#aaa;transition:all 0.15s;">📝 Blog</a>
          </div>
        </div>
      `
      ...
      el.querySelectorAll('.about-tab').forEach(tab => {
        tab.addEventListener('click', () => {
          el.querySelectorAll('.about-tab').forEach(t => t.style.background = '#1a1a30')
...
function makeTab(label) {
  ...
  return `<button class="about-tab" data-tab="${id}" style="padding:6px 12px;background:#1a1a30;border:1px solid #333;border-radius:6px;font-size:12px;color:#ccc;cursor:pointer;">${label}</button>`
}
