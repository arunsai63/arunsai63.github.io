...
export function renderContent(el) {
  el.innerHTML = `
    <div class="abt">
      ...
      <div class="abt-hero">
        ...
        <div class="abt-name">${profile.name}</div>
        <div class="abt-title">${profile.title}</div>
        <div class="abt-company">@ ${profile.company} &middot; ${profile.yoe}y</div>
        <div class="abt-links">
          <a href="${profile.github}" target="_blank" class="abt-link">
          ...
          <a href="${profile.linkedin}" target="_blank" class="abt-link">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
          </a>
          <a href="${profile.blog}" target="_blank" class="abt-link">
          ...
          <a href="mailto:${profile.email}" class="abt-link">
  ...
  // Tab switching
  el.querySelectorAll('.abt-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      el.querySelectorAll('.abt-tab').forEach(t => { t.classList.remove('abt-tab-active') })
      tab.classList.add('abt-tab-active')
      const content = el.querySelector('#about-tab-content')
      const key = tab.dataset.tab
      if (key === 'hot-takes') content.innerHTML = hotTakes()
      else if (key === 'unpopular-opinions') content.innerHTML = unpopularOpinions()
      else if (key === 'stats') content.innerHTML = stats()
      else if (key === 'quirks') content.innerHTML = quirks()
    })
  })
}
...
function stats() {
  return `<div class="abt-stats-grid">
    <div class="abt-stat"><div class="abt-stat-val" style="color:#10b981;">${getYOE()}+</div><div class="abt-stat-label">Years of Experience</div></div>
...
function quirks() {
  return `<div class="abt-quirk-list">
