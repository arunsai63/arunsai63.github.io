...
export function open() {
  createWindow({
    ...
    content: (el) => {
      ...
      el.innerHTML = `
        <div style="padding:12px 16px;background:#12122a;border-bottom:1px solid #222;">
          ...
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <input id="contact-subject" type="text" placeholder="Type or pick a suggestion..." style="flex:1;background:#1a1a30;border:1px solid #333;border-radius:4px;padding:6px 10px;color:#ccc;font-family:inherit;font-size:13px;outline:none;" />
          </div>
          <div id="subject-suggestions" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:8px;">
            ${['Job Opportunity', 'Freelance Gig', "You're a Genius", 'Found a Bug (unlikely)', 'Want to Collaborate', 'Just Saying Hi'].map(s =>
              `<button class="subject-btn" style="padding:3px 10px;background:#1a1a30;border:1px solid #2a2a3e;border-radius:12px;font-size:11px;color:#888;cursor:pointer;font-family:inherit;">${s}</button>`
            ).join('')}
          </div>
        </div>
