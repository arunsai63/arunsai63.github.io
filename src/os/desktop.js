...
export function renderDesktop(container) {
  ...
  desktopIcons.forEach(icon => {
    ...
    `
    el.addEventListener('dblclick', () => openApp(icon.id))
...
function showContextMenu(x, y) {
  ...
  const items = [
    { icon: '🖥️', label: 'Open Terminal', shortcut: 'Ctrl+T', action: () => openApp('terminal') },
    ...
    { icon: '👁️', label: 'View Source', action: () => window.open('https://github.com/arunsai63/arunsai63.github.io', '_blank') },
    { icon: 'ℹ️', label: 'About ArunOS', action: () => showAboutOS() },
  ]
  ...
  createWindow({
    ...
    content: `
      <div style="text-align:center;padding:20px;">
        ...
        <p style="color:#555;font-size:11px;margin-top:16px;">Built with mass amounts of caffeine by Arun Munaganti.<br>No frameworks were mass-harmed in the making of this OS.</p>
      </div>
    `,
  })
}
